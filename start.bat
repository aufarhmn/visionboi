...
for /f "tokens=1,* delims==" %%A in (config.env) do (
    ...
    if "%%A"=="PORT_LIDAR_2" set "PORT_LIDAR_2=%%B"
    if "%%A"=="CAMERA_ID_1" set "CAMERA_ID_1=%%B"
    if "%%A"=="CAMERA_ID_2" set "CAMERA_ID_2=%%B"
    if "%%A"=="BAUDRATE" set "BAUDRATE=%%B"
)

echo Loaded PORT_LIDAR_1=%PORT_LIDAR_1%
echo Loaded PORT_LIDAR_2=%PORT_LIDAR_2%
echo Loaded CAMERA_ID_1=%CAMERA_ID_1%
echo Loaded CAMERA_ID_2=%CAMERA_ID_2%
echo Loaded BAUDRATE=%BAUDRATE%

pushd serial-comm
start /b python mock-pub.py %PORT_LIDAR_1% %PORT_LIDAR_2% > NUL 2>&1
popd
...
start /b python dev-rec.py %CAMERA_ID_1% %CAMERA_ID_2% > NUL 2>&1
popd
